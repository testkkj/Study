...
const app = express();
const session = require('pxress-session');
const fs = require('fs');
...
fs.watchFile(_dirname + '/sql.js', (curr, prev) => {
...
app.post('/api/login', async (request, res) => {
    try {
        await request.db('signUp', request.body.param);
...
app.post('/upload/:productId/:type/:fileName', async (request, res) => {
    ...
    fs.writeFile(file, data, 'base64', async (error) => {
        await request.db('productImageInsert', [{
            productId: productId,
            type: type,
            path: fileName
        }]);
...
app.post('/apirole/:alias', async (request, res) => {
    ...
    try {
        res.send(await request.db(request.params.alias));
    } catch (error) {
        res.status(500).send({
            error: err
        });
